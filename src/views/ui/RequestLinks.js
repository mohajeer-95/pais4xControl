...
const SliderList = () => {
  ...
  const fetchSliders = async () => {
    ...
    try {
      const response = await fetch("https://paid4x.com/broker/public/api/all-brokers-link-request", requestOptions);
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const response = await fetch(`https://paid4x.com/broker/public/api/slider/${id}`, requestOptions);
      if (response.ok) {
  ...
  const updateLinkRequestStatus = async (requestId) => {
    ...
    try {
      const response = await fetch("https://paid4x.com/broker/public/api/link-request", requestOptions);
      ...
      <div className="d-lg-none">
        {(sliders || []).map(request => (
          <Card key={request.request_id} className="mb-3">
            <CardImg top
              src={`https://paid4x.com/broker/public/slider/tSdpNzscJY0XVkRfedK3BC7JtmiSAraPijVOdVrA.jpg`}
              alt={request.name} />
            <CardBody>
