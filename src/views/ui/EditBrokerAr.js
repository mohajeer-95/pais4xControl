...
const BrokerForm = () => {
    ...
    const handleBrokerSubmit = (e) => {
        ...
        const modifiedFields_ar = Object.entries(modifiedFields).reduce((acc, [key, value]) => {
            acc[`${key}_ar`] = value;
            return acc;
          }, {});
        
          console.log('modifiedFields', modifiedFields);
          console.log('modifiedFields With _ar', modifiedFields_ar);
        setSubmitting(true);
        fetch(`https://paid4x.com/broker/public/api/broker/${id}`, {
            method: "PATCH",
            headers: {
            ...
            body: JSON.stringify(modifiedFields_ar),
        })
            .then((response) => response.json())
            .then((data) => {
    ...
    const handleTradingCostSubmit = (e) => {
        ...
        const modifiedTradingCostFields_ar = Object.entries(modifiedTradingCostFields).reduce((acc, [key, value]) => {
            acc[`${key}_ar`] = value;
            return acc;
          }, {});
//here id
        ...
        fetch(`https://paid4x.com/broker/public/api/broker-trading-cost/${tradingCostId}`, {
            ...
            body: JSON.stringify(modifiedTradingCostFields_ar),
        })
            .then((response) => response.json())
            .then((data) => {
    ...
    const handleBrokerAccountSubmit = (e) => {
        ...
        const modifiedBrokerAccountFields_ar = Object.entries(modifiedBrokerAccountFields).reduce((acc, [key, value]) => {
            acc[`${key}_ar`] = value;
            return acc;
          }, {});
        ...
        fetch(`https://paid4x.com/broker/public/api/broker-account/${brokerAccountId}`, {
            ...
            body: JSON.stringify(modifiedBrokerAccountFields_ar),
        })
            .then((response) => response.json())
            .then((data) => {
    ...
    const req = {
        broker_id_ar: id,
        type_name_ar: formData.brokerType,
        forex_ar: formData.FX,
        metals_ar: formData.Metals,
        energies_ar: formData.Energies,
        indicies_ar: formData.Indicies,
        crypto_ar: formData.Stocks
    }
    ...
    const handleEnEditClick = () => () => {
        navigate(`/editBroker/${id}`);
      }
    ...
    return (
        <div>
            {loading ? (
                ...
                <>
                    <div onClick={handleEnEditClick()} style={{ cursor: "pointer", padding: 20, backgroundColor: 'rgb(38, 198, 218)', borderRadius: 9, marginTop: 10, marginBottom: 40, alignItems: 'center', textAlign: 'center' }}>
                        <h3>For English Language Click here</h3>
                    </div>
                    ...
                    {brokerType?.length && <div>
                        <h3 style={{marginTop: 155}}>Edit Account types</h3>
